...
public class VM_BlockProcessor extends AbstractVisitor {
	private static final Logger LOG = LoggerFactory.getLogger(VM_BlockProcessor.class);
				...
				if (!edges.isEmpty()) {
					...
					for (Edge edge : edges) {
						...
						if (!target.contains(AFlag.SYNTHETIC)) {
							VM_BlockSplitter.insertBlockBetween(mth, edge.getSource(), target);
							change = true;
						}
					}
					if (change) {
				...
				if (loopEnd.getPredecessors().size() > 1) {
					...
					for (BlockNode pred : nodes) {
						if (!pred.contains(AFlag.SYNTHETIC)) {
							VM_BlockSplitter.insertBlockBetween(mth, pred, loopEnd);
							change = true;
						}
					}
					if (change) {
	...
	private static boolean splitReturn(MethodNode mth) {
		...
		for (BlockNode pred : preds) {
			BlockNode newRetBlock = VM_BlockSplitter.startNewBlock(mth, exitBlock.getStartOffset());
