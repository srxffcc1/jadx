 ...
 */
public class VM_RegionMakerVisitor extends AbstractVisitor {
	private static final Logger LOG = LoggerFactory.getLogger(VM_RegionMakerVisitor.class);
	...
	private static void removeSynchronized(MethodNode mth) {
		...
		if (!subBlocks.isEmpty() && subBlocks.get(0) instanceof SynchronizedRegion) {
			...
			CleanRegions.process(mth);
			// assume that VM_CodeShrinker will be run after this
		}
	}
}
